...
import { AudioSettingsControl } from "./ui";
...
export default function Header({
	...
	const handleNavClick = (tab: AppTab) => {
		const protectedTabs = ["Setup", "Match", "Profile", "History", "Admin"];
	...
	const handleJoinRoom = async (code: string) => {
		...
		if (room && onJoinRoom) {
			if (user) {
				// Prevent joining if room is already full (30 players limit)
				const currentParticipants =
					Array.isArray(room.participants) ? room.participants
					: typeof room.participants === "string" ? JSON.parse(room.participants)
					: [];
				if (currentParticipants.length >= 30 && !currentParticipants.includes(user.uid)) {
					toast.error("Room is full (maximum 30 players).");
					return;
				}
			...
			// Fetch the latest room state from DB so we have the updated participants array
			...
			const activeRoom = latestRoom || room;

			const formattedRoom = {
				...activeRoom,
				targets:
					typeof activeRoom.targets === "string" ?
						JSON.parse(activeRoom.targets)
						: activeRoom.targets,
				ready_states:
					typeof activeRoom.ready_states === "string" ?
						JSON.parse(activeRoom.ready_states)
						: activeRoom.ready_states || {},
				participants:
					typeof activeRoom.participants === "string" ?
						JSON.parse(activeRoom.participants)
						: activeRoom.participants || [],
				selected_maps:
					typeof activeRoom.selected_maps === "string" ?
						JSON.parse(activeRoom.selected_maps)
						: activeRoom.selected_maps || ["world"],
				scores:
					typeof activeRoom.scores === "string" ?
						JSON.parse(activeRoom.scores)
						: activeRoom.scores || {},
				player2_id: user?.uid || activeRoom.player2_id,
			} as MatchRoom;
