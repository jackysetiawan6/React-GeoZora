...
export default function Leaderboard() {
	...
	useEffect(() => {
		...
		const cleanupGuests = async () => {
			try {
				// Cleanup guests inactive for more than 10 minutes
				await supabase.rpc("cleanup_inactive_guests", { p_minutes: 10 });
			} catch (err) {
		...
		const fetchLeaderboard = async (isBackground = false) => {
			...
			try {
				if (filter === "overall" && metric === "elo") {
					// Overall ELO - ranking by profiles.elo, excluding guests
					const { data, error } = await supabase
						.from("profiles")
						.select(
							"id, display_name, elo, exp, games_played, email, avatar_url",
						)
						.not("email", "is", null) // Exclude guests
						.gt("games_played", 0)
					...
					if (!error && data && data.length > 0) {
						setEntries(
							data.map((p, index) => {
								const rounds = p.games_played || 0;
								const totalScore = p.elo || 1300;
								return {
									id: p.id,
									rank: index + 1,
									...
									avg_score: rounds > 0 ? Math.round((p.exp || 0) / rounds) : 0,
				...
				} else if (filter === "overall" && metric === "exp") {
					// Overall EXP - excluding guests
					const { data, error } = await supabase
						.from("profiles")
						.select(
							"id, display_name, exp, elo, games_played, email, avatar_url",
						)
						.not("email", "is", null) // Exclude guests
						.gt("games_played", 0)
						...
						setEntries(
							data.map((p, index) => {
								...
								return {
									...
									elo: p.elo || 1300,
									rounds_played: rounds,
									avg_score: rounds > 0 ? Math.round(totalExp / rounds) : 0,
									avatar: p.avatar_url || undefined,
									isCurrentUser: user?.uid === p.id,
								};
							}),
					...
					// For time-based filters, we query exp_history
					const now = new Date();
					const fromDate = new Date();
					if (filter === "today") fromDate.setDate(now.getDate() - 1);
					if (filter === "week") fromDate.setDate(now.getDate() - 7);
					if (filter === "month") fromDate.setMonth(now.getMonth() - 1);
					if (filter === "year") fromDate.setFullYear(now.getFullYear() - 1);

					const { data: expEvents, error: expError } = await supabase
						.from("exp_history")
						.select("user_id, exp_gained")
						.gte("created_at", fromDate.toISOString());

					if (!expError && expEvents && expEvents.length > 0) {
						const statsMap: Record<string, { exp: number; rounds: number }> =
							{};
						expEvents.forEach((ev: any) => {
							if (!statsMap[ev.user_id])
								statsMap[ev.user_id] = { exp: 0, rounds: 0 };
							statsMap[ev.user_id].exp += ev.exp_gained;
							statsMap[ev.user_id].rounds += 1;
						});
						...
						if (userIds.length > 0) {
							const { data: profiles, error: pError } = await supabase
								.from("profiles")
								.select("id, display_name, elo, email, avatar_url")
								.not("email", "is", null) // Exclude guests
								.in("id", userIds);

							if (!pError && profiles) {
								const finalEntries = profiles
									.map(p => ({
										id: p.id,
										display_name: p.display_name,
										total_exp: statsMap[p.id].exp,
										rounds: statsMap[p.id].rounds,
										elo: p.elo,
										avatar: p.avatar_url,
									}))
								...
								setEntries(
									finalEntries.map((p, index) => ({
										...
										avg_score:
											p.rounds > 0 ? Math.round(p.total_exp / p.rounds) : 0,
					...
					<div className="flex flex-col gap-3">
						...
						<div className="flex bg-[var(--color-app-panel)] border border-[var(--color-app-border-light)] p-1 rounded-xl">
							<button
								...
								className={cn(
									"flex-1 px-4 py-2 text-sm font-bold rounded-lg transition-all border border-transparent",
									metric === "exp" ?
										"bg-[var(--color-app-hover)] text-white shadow shadow-black/20 border-[var(--color-app-border)]"
							...
							{(Object.keys(filterLabels) as TimeFilter[]).map(key => (
								<button
									...
									className={cn(
										...
										:	"text-[var(--color-app-text-muted)] hover:text-white hover:bg-[var(--color-app-hover)] border border-transparent",
									)}>
									{filterLabels[key]}
									{filter === key && (
				...
				:	<div className="flex-1 flex flex-col overflow-y-auto no-scrollbar scroll-smooth pr-1 lg:pr-4 pb-12 pt-4">
							...
							{topThree[0] && (
								<div className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-10 duration-700 delay-150 w-1/3 max-w-[220px] -mt-10 lg:-mt-16 z-10 pb-4">
									...
									<div className="text-center px-2 w-full">
										<h3 className="font-bold text-white text-base lg:text-xl truncate mb-1.5">
						...
						{remaining.length > 0 && (
							<div className="bg-[var(--color-app-panel)] border border-[var(--color-app-border-light)] rounded-2xl shadow-xl overflow-hidden mt-4 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-500">
								{/* Table Header */}
								<div className="grid grid-cols-12 gap-4 items-center px-6 py-4 border-b border-[var(--color-app-border-light)] text-xs font-black uppercase tracking-widest text-[var(--color-app-text-muted)] bg-white/5">
