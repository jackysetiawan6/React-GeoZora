...
export function useDoubleBufferedStreetView({
  ...
  const loadIntoSlot = useCallback(
    async (slot: PanoSlot, round: number, target: StreetViewTarget) => {
      ...
      const restrictedKeyboard = noMoving || noPanning || noZooming;
      ...
      const options = {
        ...
        draggable: !noPanning,
        keyboardShortcuts: !restrictedKeyboard,
      };
      ...
      const waitUntilReady = new Promise<void>((resolve) => {
        ...
        listenerRefs.current[slot] = [statusListener, tilesListener, povListener];
