				...
				{isCreator ?
					<>
						<NumericInput
							...
							max={30}
							value={customRounds}
							onChange={val => setCustomRounds(val === "" ? 5 : Number(val))}
						/>
						<NumericInput
							label="Time"
							min={20}
							...
							step={5}
							value={customSeconds}
							onChange={val => setCustomSeconds(val === "" ? 20 : Number(val))}
							suffix="s"
						/>
				...
				<section className="bg-[var(--color-app-panel)] border border-[var(--color-app-border-light)] rounded-2xl shadow-xl overflow-hidden p-6 lg:p-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 border-b border-[var(--color-app-border-light)] pb-4">
						...
						<button
							...
							className={cn(
								"px-7 py-3 rounded-xl font-bold transition-all shadow-lg text-sm uppercase tracking-wider text-white shrink-0",
								startDisabled ?
									"bg-gray-700/50 cursor-not-allowed border-transparent shadow-none text-gray-400"
								:	"bg-[var(--color-app-blue)] hover:bg-blue-500 hover:shadow-[0_0_20px_rgba(59,130,246,0.4)]",
							)}>
							{startLabel}
						</button>
					</div>
								...
								return (
									<button
										...
										<div className="relative z-10 flex flex-col items-center justify-center text-center">
											<div className="text-4xl mb-3 drop-shadow-lg transform transition-transform group-hover:scale-110">
												{map.flag}
											</div>
											<div
												className={cn(
													"font-bold text-sm leading-tight drop-shadow-md",
											...
											{FALLBACK_LOCATIONS[mapKey] && (
												<div className="mt-1 text-[10px] font-medium text-[var(--color-app-text-muted)] tracking-wide uppercase">
