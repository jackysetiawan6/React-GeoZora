...
function drawOpponentResult(
  mapInstance: any,
  result: { guess: LatLng; target: LatLng; playerIndex: number; round?: number; avatarUrl?: string | null },
  bounds: any | null,
  markersArray: any[],
  linesArray: any[],
  viewBounds: any
...
export default function GuessMiniMap({
  ...
  const smoothCameraCancelRef = useRef<(() => void) | null>(null);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      removeMarker(guessMarker.current);
      removeMarker(targetMarker.current);
      routeLine.current?.setMap?.(null);
      regionHighlight.current?.setMap?.(null);
      historyMarkers.current.forEach(removeMarker);
  ...
  useEffect(() => {
    ...
    if (phase !== 'reveal' || !target) {
      removeMarker(targetMarker.current);
      routeLine.current?.setMap?.(null);
      otherGuessMarkers.current.forEach(removeMarker);
    ...
    if (guess) {
      ...
      timerId1 = window.setTimeout(() => {
        ...
        smoothCameraCancelRef.current = smoothCamera(
          mapInstance.current,
          from,
          overview,
          overviewDuration,
      ...
      timerId3 = window.setTimeout(() => {
        ...
        smoothCameraCancelRef.current = smoothCamera(mapInstance.current, from, dest, duration, isActive);
      }, 300);
    }

    return () => {
      if (timerId1 !== null) clearTimeout(timerId1);
  ...
  return (
    <div className="w-full h-full min-h-[220px] rounded-xl overflow-hidden bg-slate-200 relative group">
      ...
      {(disabled || submitting || phase === 'waiting_for_others') && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/50 backdrop-blur-[3px] transition-all duration-300">
          <div className="w-10 h-10 border-4 border-white/30 border-t-white rounded-full animate-spin mb-3" />
          <span className="text-white text-xs font-semibold tracking-wide px-3 py-1 bg-black/45 rounded-full border border-white/10 mb-2">
            {phase === 'waiting_for_others' ? 'Waiting for other players...' : 'Submitting guess...'}
          </span>
          {phase === 'waiting_for_others' && waitingForPlayers.length > 0 && (
            <div className="text-center px-4 max-w-[90%]">
              <span className="text-[10px] uppercase font-bold text-white/50 tracking-wider">Pending</span>
              <div className="text-white/90 text-[11px] font-medium mt-0.5 truncate max-w-full">
