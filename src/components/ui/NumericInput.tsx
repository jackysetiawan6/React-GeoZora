...
export function NumericInput({
	...
	// Sync local state with prop value when prop changes
	...
	const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (!isNaN(num)) {
			// Call onChange with intermediate numeric value so parent state receives it
			onChange(num);
		}
	};
	...
	const handleBlur = () => {
		if (localValue === "") {
			const fallback = min !== undefined ? min : "";
			onChange(fallback);
			setLocalValue(fallback !== "" ? String(fallback) : "");
			return;
		}
		...
		if (step !== undefined && step > 0) {
			...
			// Re-clamp in case step rounding pushed it out of bounds
			if (min !== undefined && num < min) num = min;
			if (max !== undefined && num > max) num = max;
		}
