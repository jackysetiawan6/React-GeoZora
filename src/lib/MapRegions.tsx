...
/**
 * Recursively fetches all fallback locations page-by-page in chunks of 1000 rows
 * to cleanly bypass any PostgREST server-side Max Rows / pagination constraints.
 */
async function fetchAllFallbackLocations(): Promise<FallbackLocationRow[]> {
  const allLocations: FallbackLocationRow[] = [];
  let from = 0;
  const pageSize = 1000;
  let hasMore = true;
  ...
  while (hasMore) {
    ...
    if (data && data.length > 0) {
      allLocations.push(...(data as FallbackLocationRow[]));
...
export async function loadMapRegions(): Promise<void> {
  ...
  _loadPromise = (async () => {
    const [regionsRes, locations] = await Promise.all([
      supabase
        .from('map_regions')
        .select('*')
        .eq('is_enabled', true)
        .order('sort_order', { ascending: true }),
      fetchAllFallbackLocations(),
    ]);
