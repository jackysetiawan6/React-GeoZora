...
export async function batchUpdateReactions(
	...
	results.forEach((result, index) => {
		if (result.status === "fulfilled" && result.value.error) {
			const dbErr = result.value.error;
			console.error(
				`Failed to update reaction for message ${reactions[index].messageId}:`,
				dbErr,
			);
			void logSystemError("Failed to update message reaction", {
				messageId: reactions[index].messageId,
				error: dbErr.message,
				code: dbErr.code,
				roomId,
			});
			...
			void logSystemError("Rejected reaction update promise", {
				messageId: reactions[index].messageId,
